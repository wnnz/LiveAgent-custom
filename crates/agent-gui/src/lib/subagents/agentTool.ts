...
export function createSubagentTools(params: {
  ...
  const toolAgent: Tool = {
    ...
    description: [
      ...
      "Each agent has a stable `id` inside this conversation. Reuse the same id only when its stored persona and template still fit the job; use a new id when the required role or template differs.",
      "Creation fields (name, role, identity, template) apply only when an id is first created; sending different values for an existing id is an error. For a compatible existing id, send only id and the new prompt. A task prompt cannot override a wrongly bound template.",
      ...
      "Choose each new agent's template by the requested final deliverable, not by task complexity or incidental keywords. Omit template only when no configured role is suitable.",
      "If the deliverable requires implementing, modifying, fixing, or testing code, select the matching development template. Backend code, APIs, persistence, transactions, or migrations use a backend-development role even when architecture analysis is also required.",
      "Use an architecture/design template only when the requested deliverable is analysis, design, planning, or review and no code implementation is required.",
      "Agents that must edit files, compile, or run implementation tests need mode=worktree. Use mode=readonly only for inspection, research, review, or other work that does not modify the repository.",

...
export function snapshotToolCallForTrace(toolCall: ToolCall): ToolCall {
  const args =
    toolCall.arguments &&
...
export function nextTextLikeBlockId(blocks: UiRoundContentBlock[], kind: "thinking" | "text") {
  let max = 0;
...
export function appendTextLikeBlock(
  blocks: UiRoundContentBlock[],
  kind: "thinking" | "text",
  delta: string,
) {
...
export function updateLiveRound<TRound extends { round: number }>(
  previousRounds: TRound[],
  round: number,
  updater: (target: TRound) => TRound,
) {
  if (previousRounds.length === 0) return previousRounds;

  const lastIndex = previousRounds.length - 1;
  if (previousRounds[lastIndex].round === round) {
    const next = previousRounds.slice();
    next[lastIndex] = updater(previousRounds[lastIndex]);
    return next;
  }

  const index = previousRounds.findIndex((item) => item.round === round);
  if (index < 0) return previousRounds;

  const next = previousRounds.slice();
  next[index] = updater(previousRounds[index]);
  return next;
}

export function collapseThinking<
  TRound extends Pick<UiRound, "blocks"> & { thinkingOpen: boolean },
>(target: TRound): TRound {
